...
//     const indent = await SalesRequest.findOne({ _id: id });
//     console.log('Found indent for status update:', indent);

  
//    await SalesRequest.updateOne({ _id: id }, { $set: updateData });
 
//     res.status(200).json({ message: `Status updated for indent ${id}` });
//   } catch (err) {
...
const SalesRequest = require('../../models/sales/Salesrequest');
const Salecategory = require('../../models/categories/SaleContractCategoryModel');

exports.createIndent = async (req, res) => {
  try {
    const { indentIdType, externalIndentId, categoryId, items } = req.body;

    // console.log('=== CREATE INDENT CALLED ===', { indentIdType, externalIndentId, categoryId, items });
    let indentId;
    console.log('Received categoryId:', categoryId);
    console.log('Received items createIndent:', req.body);
    ...
    if (indentIdType === 'external') {
      ...
      if (!category.rangeFrom) {
        return res.status(400).json({ error: 'Category range start not defined' });
      }

      const rangeStart = category.rangeFrom;
      console.log('Category rangeStart:', rangeStart);

      // Find all indents for this category (removed indentIdType filter)
      const existingIndents = await SalesRequest.find({
        categoryId
      }).select('indentId');
      ...
      if (existingIndents.length > 0) {
        ...
        const usedNumbers = existingIndents
          .map(indent => {
            ...
            const parsed = parseInt(indent.indentId, 10);
            return isNaN(parsed) ? null : parsed;
          })
          .filter(num => num !== null && num >= rangeStart && (!category.rangeEnd || num <= category.rangeEnd));
      ...
      // Check range limit if category has rangeEnd
      if (category.rangeEnd && nextNumber > category.rangeEnd) {
        return res.status(400).json({ 
          error: `Indent number exceeded category range. Next: ${nextNumber}, Max: ${category.rangeEnd}` 
        });
